...
int main(int argc, char* argv[]) {
    ...
    SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO); // Make sure AUDIO is included here
    IMG_Init(IMG_INIT_PNG);

    if (!initAudio()) {
        std::cout << "Warning: Audio failed to initialize!" << std::endl;
    }
    ...
    SDL_Texture* tPop = IMG_LoadTexture(ren, "pop_btn.jpg");
    ...
    // FIXED: Added {NEXT_COSTUME, tNextCostume} to the palette list!
    standardItems = {
            ...
            {WAIT_SEC, tWait}, {REPEAT_BEGIN, tRepStart}, {REPEAT_END, tRepEnd}, {TIMER_RESET, tTRes}, {TIMER_SAY, tTSay} , {PLAY_POP, tPop}
    };
    ...
    while (!quit) {
        ...
        if ((isRunning || runNextBlock) && pc < workspace.size()) {
            ...
            if (b.action == REPEAT_BEGIN) {
                ...
                else if (b.action == PLAY_POP) {
                    playPop();
                    logAction("Played pop sound.");
                    renderAll(ren, player, workspace, nullptr, isRunning, tNew, tExt, tStop, resetBtn, startBtn, stopBtn, extBtn, canvas, backdrops, activeBG, tPausedMsg, showPausedMsg);
                    SDL_Delay(100);
                }
                else if (b.action == NEXT_COSTUME) {
                    // FIXED: Added safety check to prevent crash if no costumes are loaded
